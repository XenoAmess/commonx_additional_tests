...
public class CharToArrayTest {

    static class CharTestCollection implements AbstractCharCollection {
        private final char[] elements;
        private int[] sizes;
        private int nextSize;

        public CharTestCollection(char[] elements) {
    ...
    static final char[] OBJECTS = {(char) 0, (char) 1, (char) 2};
    static final CharTestCollection CANDIDATE = new CharTestCollection(OBJECTS);
    ...
    protected void test() throws Throwable {
        ...
        res = new CharTestCollection(new char[]{1, 2}).toArrayPrimitive(new char[0]);
        assertTrue(res instanceof char[]);
        assertTrue(res.length == 2);
        assertTrue(res[1] == (char) 2);

        // Check incompatible type of target array

        res = CANDIDATE.toArrayPrimitive(new char[CAP]);

        // Check more elements than a.length
        a = new char[CAP - 1]; // appears too small
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res != a);
        assertTrue(res[LAST] != (char) 0);

        // Check equal elements as a.length
        a = new char[CAP]; // appears to match
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res == a);
        assertTrue(res[last()] != (char) 0);

        // Check equal elements as a.length
        a = new char[CAP + 1]; // appears too big
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res == a);
        assertTrue(res[last()] == (char) 0);

        // Check less elements than expected, but more than a.length
        a = new char[CAP - 2]; // appears too small
        CANDIDATE.setSizeSequence(CAP, CAP - 1);
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res != a);
        assertTrue(res.length == CAP - 1);
        assertTrue(res[LAST - 1] != (char) 0);
        ...
        a = new char[CAP - 1]; // appears to match
