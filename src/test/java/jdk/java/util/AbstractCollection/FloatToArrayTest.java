...
public class FloatToArrayTest {

    static class FloatTestCollection implements AbstractFloatCollection {
        private final float[] elements;
        private int[] sizes;
        private int nextSize;

        public FloatTestCollection(float[] elements) {
    ...
    static final float[] OBJECTS = {(float) 0, (float) 1, (float) 2};
    static final FloatTestCollection CANDIDATE = new FloatTestCollection(OBJECTS);
    ...
    protected void test() throws Throwable {
        ...
        res = new FloatTestCollection(new float[]{1, 2}).toArrayPrimitive(new float[0]);
        assertTrue(res instanceof float[]);
        assertTrue(res.length == 2);
        assertTrue(res[1] == (float) 2);

        // Check incompatible type of target array

        res = CANDIDATE.toArrayPrimitive(new float[CAP]);

        // Check more elements than a.length
        a = new float[CAP - 1]; // appears too small
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res != a);
        assertTrue(res[LAST] != (float) 0);

        // Check equal elements as a.length
        a = new float[CAP]; // appears to match
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res == a);
        assertTrue(res[last()] != (float) 0);

        // Check equal elements as a.length
        a = new float[CAP + 1]; // appears too big
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res == a);
        assertTrue(res[last()] == (float) 0);

        // Check less elements than expected, but more than a.length
        a = new float[CAP - 2]; // appears too small
        CANDIDATE.setSizeSequence(CAP, CAP - 1);
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res != a);
        assertTrue(res.length == CAP - 1);
        assertTrue(res[LAST - 1] != (float) 0);
        ...
        a = new float[CAP - 1]; // appears to match
