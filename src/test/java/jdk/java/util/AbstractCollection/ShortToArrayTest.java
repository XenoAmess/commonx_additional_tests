...
public class ShortToArrayTest {

    static class ShortTestCollection implements AbstractShortCollection {
        private final short[] elements;
        private int[] sizes;
        private int nextSize;

        public ShortTestCollection(short[] elements) {
    ...
    static final short[] OBJECTS = {(short) 0, (short) 1, (short) 2};
    static final ShortTestCollection CANDIDATE = new ShortTestCollection(OBJECTS);
    ...
    protected void test() throws Throwable {
        ...
        res = new ShortTestCollection(new short[]{1, 2}).toArrayPrimitive(new short[0]);
        assertTrue(res instanceof short[]);
        assertTrue(res.length == 2);
        assertTrue(res[1] == (short) 2);

        // Check incompatible type of target array

        res = CANDIDATE.toArrayPrimitive(new short[CAP]);

        // Check more elements than a.length
        a = new short[CAP - 1]; // appears too small
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res != a);
        assertTrue(res[LAST] != (short) 0);

        // Check equal elements as a.length
        a = new short[CAP]; // appears to match
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res == a);
        assertTrue(res[last()] != (short) 0);

        // Check equal elements as a.length
        a = new short[CAP + 1]; // appears too big
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res == a);
        assertTrue(res[last()] == (short) 0);

        // Check less elements than expected, but more than a.length
        a = new short[CAP - 2]; // appears too small
        CANDIDATE.setSizeSequence(CAP, CAP - 1);
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res != a);
        assertTrue(res.length == CAP - 1);
        assertTrue(res[LAST - 1] != (short) 0);
        ...
        a = new short[CAP - 1]; // appears to match
