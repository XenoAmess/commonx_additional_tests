...
public class ByteToArrayTest {

    static class ByteTestCollection implements AbstractByteCollection {
        private final byte[] elements;
        private int[] sizes;
        private int nextSize;

        public ByteTestCollection(byte[] elements) {
    ...
    static final byte[] OBJECTS = {(byte) 0, (byte) 1, (byte) 2};
    static final ByteTestCollection CANDIDATE = new ByteTestCollection(OBJECTS);
    ...
    protected void test() throws Throwable {
        ...
        res = new ByteTestCollection(new byte[]{1, 2}).toArrayPrimitive(new byte[0]);
        assertTrue(res instanceof byte[]);
        assertTrue(res.length == 2);
        assertTrue(res[1] == (byte) 2);

        // Check incompatible type of target array

        res = CANDIDATE.toArrayPrimitive(new byte[CAP]);

        // Check more elements than a.length
        a = new byte[CAP - 1]; // appears too small
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res != a);
        assertTrue(res[LAST] != (byte) 0);

        // Check equal elements as a.length
        a = new byte[CAP]; // appears to match
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res == a);
        assertTrue(res[last()] != (byte) 0);

        // Check equal elements as a.length
        a = new byte[CAP + 1]; // appears too big
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res == a);
        assertTrue(res[last()] == (byte) 0);

        // Check less elements than expected, but more than a.length
        a = new byte[CAP - 2]; // appears too small
        CANDIDATE.setSizeSequence(CAP, CAP - 1);
        res = CANDIDATE.toArrayPrimitive(a);
        assertTrue(res != a);
        assertTrue(res.length == CAP - 1);
        assertTrue(res[LAST - 1] != (byte) 0);
        ...
        a = new byte[CAP - 1]; // appears to match
