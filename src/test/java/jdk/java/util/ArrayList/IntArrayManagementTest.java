...
public class IntArrayManagementTest {
    ...
    static {
        try {
            ELEMENT_DATA = IntArrayList.class.getDeclaredField("elementData");
            ELEMENT_DATA.setAccessible(true);
            MODCOUNT = AbstractIntList.class.getDeclaredField("modCount");
    ...
    static int[] elementData(IntArrayList list) {
        try {
            return (int[]) ELEMENT_DATA.get(list);
        } catch (ReflectiveOperationException huh) {
            throw new AssertionError(huh);
        }
    }

    static int modCount(IntArrayList list) {
        try {
    ...
    static int capacity(IntArrayList list) {
    ...
    static void ensureCapacity(IntArrayList list, int capacity) {
    ...
    public void emptyArraysAreShared() {
        assertSame(elementData(new IntArrayList()),
                elementData(new IntArrayList()));
        assertSame(elementData(new IntArrayList(0)),
                elementData(new IntArrayList(0)));
    }

    @Test
    public void emptyArraysDifferBetweenDefaultAndExplicit() {
        assertNotSame(elementData(new IntArrayList()),
                elementData(new IntArrayList(0)));
    }
