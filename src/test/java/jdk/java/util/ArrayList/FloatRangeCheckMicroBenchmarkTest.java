...
public class FloatRangeCheckMicroBenchmarkTest {
    ...
    private static void time(Job... jobs) throws Throwable {
        ...
        for (int i = 0; i < jobs.length; i++) {
            ...
            ratios[i] = (float) ((float) nanoss[i] / (float) nanoss[0]);
    ...
    private static void deoptimize(FloatArrayList list) {
        for (Float x : list)
            if (x == null)
                throw new Error();
    }
    ...
    public static void main(String[] args) throws Throwable {
        ...
        final Job[] jobs = {
                ...
                new Job("set") {
                    void work() {
                        Float[] xs = list.toArray(new Float[size]);
                        for (int i = 0; i < iterations; i++) {
                            for (int k = 0; k < size; k++)
                                list.set(k, xs[k]);
                        }
                ...
                new Job("subList get") {
                    void work() {
                        FloatList sublist = list.subList(0, list.size());
                        for (int i = 0; i < iterations; i++) {
                            for (int k = 0; k < size; k++)
                                if (sublist.get(k) == 42)
                                    throw new Error();
                ...
                new Job("subList set") {
                    void work() {
                        FloatList sublist = list.subList(0, list.size());
                        Float[] xs = sublist.toArray(new Float[size]);
                        for (int i = 0; i < iterations; i++) {
                            for (int k = 0; k < size; k++)
                                sublist.set(k, xs[k]);
                        }
