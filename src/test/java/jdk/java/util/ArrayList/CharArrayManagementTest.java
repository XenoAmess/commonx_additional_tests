...
import com.xenoamess.commons.primitive.collections.lists.array_lists.CharArrayList;
...
public class CharArrayManagementTest {
    ...
    static {
        try {
            ELEMENT_DATA = CharArrayList.class.getDeclaredField("elementData");
            ELEMENT_DATA.setAccessible(true);
            MODCOUNT = AbstractCharList.class.getDeclaredField("modCount");
    ...
    static char[] elementData(CharArrayList list) {
        try {
            return (char[]) ELEMENT_DATA.get(list);
        } catch (ReflectiveOperationException huh) {
            throw new AssertionError(huh);
        }
    }

    static int modCount(CharArrayList list) {
        try {
    ...
    static int capacity(CharArrayList list) {
    ...
    static void ensureCapacity(CharArrayList list, int capacity) {
    ...
    public void emptyArraysAreShared() {
        assertSame(elementData(new CharArrayList()),
                elementData(new CharArrayList()));
        assertSame(elementData(new CharArrayList(0)),
                elementData(new CharArrayList(0)));
    }

    @Test
    public void emptyArraysDifferBetweenDefaultAndExplicit() {
        assertNotSame(elementData(new CharArrayList()),
                elementData(new CharArrayList(0)));
    }
