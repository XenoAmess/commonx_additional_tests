...
public class ByteArrayManagementTest {
    ...
    static {
        try {
            ELEMENT_DATA = ByteArrayList.class.getDeclaredField("elementData");
            ELEMENT_DATA.setAccessible(true);
            MODCOUNT = AbstractByteList.class.getDeclaredField("modCount");
    ...
    static byte[] elementData(ByteArrayList list) {
        try {
            return (byte[]) ELEMENT_DATA.get(list);
        } catch (ReflectiveOperationException huh) {
            throw new AssertionError(huh);
        }
    }

    static int modCount(ByteArrayList list) {
        try {
    ...
    static int capacity(ByteArrayList list) {
    ...
    static void ensureCapacity(ByteArrayList list, int capacity) {
    ...
    public void emptyArraysAreShared() {
        assertSame(elementData(new ByteArrayList()),
                elementData(new ByteArrayList()));
        assertSame(elementData(new ByteArrayList(0)),
                elementData(new ByteArrayList(0)));
    }

    @Test
    public void emptyArraysDifferBetweenDefaultAndExplicit() {
        assertNotSame(elementData(new ByteArrayList()),
                elementData(new ByteArrayList(0)));
    }
