...
public class DoubleRangeCheckMicroBenchmarkTest {
    ...
    private static void time(Job... jobs) throws Throwable {
        ...
        for (int i = 0; i < jobs.length; i++) {
            ...
            ratios[i] = (float) ((double) nanoss[i] / (double) nanoss[0]);
    ...
    private static void deoptimize(DoubleArrayList list) {
        for (Double x : list)
            if (x == null)
                throw new Error();
    }
    ...
    public static void main(String[] args) throws Throwable {
        ...
        final Job[] jobs = {
                ...
                new Job("set") {
                    void work() {
                        Double[] xs = list.toArray(new Double[size]);
                        for (int i = 0; i < iterations; i++) {
                            for (int k = 0; k < size; k++)
                                list.set(k, xs[k]);
                        }
                ...
                new Job("subList get") {
                    void work() {
                        DoubleList sublist = list.subList(0, list.size());
                        for (int i = 0; i < iterations; i++) {
                            for (int k = 0; k < size; k++)
                                if (sublist.get(k) == 42)
                                    throw new Error();
                ...
                new Job("subList set") {
                    void work() {
                        DoubleList sublist = list.subList(0, list.size());
                        Double[] xs = sublist.toArray(new Double[size]);
                        for (int i = 0; i < iterations; i++) {
                            for (int k = 0; k < size; k++)
                                sublist.set(k, xs[k]);
                        }
