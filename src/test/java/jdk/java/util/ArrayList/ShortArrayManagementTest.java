...
public class ShortArrayManagementTest {
    ...
    static {
        try {
            ELEMENT_DATA = ShortArrayList.class.getDeclaredField("elementData");
            ELEMENT_DATA.setAccessible(true);
            MODCOUNT = AbstractShortList.class.getDeclaredField("modCount");
    ...
    static short[] elementData(ShortArrayList list) {
        try {
            return (short[]) ELEMENT_DATA.get(list);
        } catch (ReflectiveOperationException huh) {
            throw new AssertionError(huh);
        }
    }

    static int modCount(ShortArrayList list) {
        try {
    ...
    static int capacity(ShortArrayList list) {
    ...
    static void ensureCapacity(ShortArrayList list, int capacity) {
    ...
    public void emptyArraysAreShared() {
        assertSame(elementData(new ShortArrayList()),
                elementData(new ShortArrayList()));
        assertSame(elementData(new ShortArrayList(0)),
                elementData(new ShortArrayList(0)));
    }

    @Test
    public void emptyArraysDifferBetweenDefaultAndExplicit() {
        assertNotSame(elementData(new ShortArrayList()),
                elementData(new ShortArrayList(0)));
    }
