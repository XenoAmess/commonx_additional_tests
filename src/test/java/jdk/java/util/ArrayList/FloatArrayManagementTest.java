...
public class FloatArrayManagementTest {
    ...
    static {
        try {
            ELEMENT_DATA = FloatArrayList.class.getDeclaredField("elementData");
            ELEMENT_DATA.setAccessible(true);
            MODCOUNT = AbstractFloatList.class.getDeclaredField("modCount");
    ...
    static float[] elementData(FloatArrayList list) {
        try {
            return (float[]) ELEMENT_DATA.get(list);
        } catch (ReflectiveOperationException huh) {
            throw new AssertionError(huh);
        }
    }

    static int modCount(FloatArrayList list) {
        try {
    ...
    static int capacity(FloatArrayList list) {
    ...
    static void ensureCapacity(FloatArrayList list, int capacity) {
    ...
    public void emptyArraysAreShared() {
        assertSame(elementData(new FloatArrayList()),
                elementData(new FloatArrayList()));
        assertSame(elementData(new FloatArrayList(0)),
                elementData(new FloatArrayList(0)));
    }

    @Test
    public void emptyArraysDifferBetweenDefaultAndExplicit() {
        assertNotSame(elementData(new FloatArrayList()),
                elementData(new FloatArrayList(0)));
    }
