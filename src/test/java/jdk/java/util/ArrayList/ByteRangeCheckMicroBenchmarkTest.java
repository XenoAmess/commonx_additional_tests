...
public class ByteRangeCheckMicroBenchmarkTest {
    ...
    private static void time(Job... jobs) throws Throwable {
        ...
        for (int i = 0; i < jobs.length; i++) {
            ...
            ratios[i] = (float) ((byte) nanoss[i] / (byte) nanoss[0]);
    ...
    private static void deoptimize(ByteArrayList list) {
        for (Byte x : list)
            if (x == null)
                throw new Error();
    }
    ...
    public static void main(String[] args) throws Throwable {
        ...
        final Job[] jobs = {
                ...
                new Job("set") {
                    void work() {
                        Byte[] xs = list.toArray(new Byte[size]);
                        for (int i = 0; i < iterations; i++) {
                            for (int k = 0; k < size; k++)
                                list.set(k, xs[k]);
                        }
                ...
                new Job("subList get") {
                    void work() {
                        ByteList sublist = list.subList(0, list.size());
                        for (int i = 0; i < iterations; i++) {
                            for (int k = 0; k < size; k++)
                                if (sublist.get(k) == 42)
                                    throw new Error();
                ...
                new Job("subList set") {
                    void work() {
                        ByteList sublist = list.subList(0, list.size());
                        Byte[] xs = sublist.toArray(new Byte[size]);
                        for (int i = 0; i < iterations; i++) {
                            for (int k = 0; k < size; k++)
                                sublist.set(k, xs[k]);
                        }
