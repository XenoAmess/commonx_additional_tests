...
public class DoubleArrayManagementTest {
    ...
    static {
        try {
            ELEMENT_DATA = DoubleArrayList.class.getDeclaredField("elementData");
            ELEMENT_DATA.setAccessible(true);
            MODCOUNT = AbstractDoubleList.class.getDeclaredField("modCount");
    ...
    static double[] elementData(DoubleArrayList list) {
        try {
            return (double[]) ELEMENT_DATA.get(list);
        } catch (ReflectiveOperationException huh) {
            throw new AssertionError(huh);
        }
    }

    static int modCount(DoubleArrayList list) {
        try {
    ...
    static int capacity(DoubleArrayList list) {
    ...
    static void ensureCapacity(DoubleArrayList list, int capacity) {
    ...
    public void emptyArraysAreShared() {
        assertSame(elementData(new DoubleArrayList()),
                elementData(new DoubleArrayList()));
        assertSame(elementData(new DoubleArrayList(0)),
                elementData(new DoubleArrayList(0)));
    }

    @Test
    public void emptyArraysDifferBetweenDefaultAndExplicit() {
        assertNotSame(elementData(new DoubleArrayList()),
                elementData(new DoubleArrayList(0)));
    }
